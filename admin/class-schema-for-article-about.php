...
class Schema_For_Article_About {
	...
	private function more_plugins() {
		$plugin_url = plugins_url( '/admin', SCHEMA_FOR_ARTICLE_FILE );
		$img_src    = $plugin_url . '/images';
		wp_enqueue_style( 'style', $plugin_url . '/css/about-plugins.min.css' );
		$plugin_name = __( 'SCHEMA for Article', 'schema-for-article' );
		$button_text = __( 'Check it out', 'schema-for-article' );
		...
		<div class="wrap">
			<div class="float">
				<h1><?php echo $plugin_name . ' ' . SCHEMA_FOR_ARTICLE_PLUGIN_VERSION; ?></h1>
				<div class="tagline">
					<p><?php _e('Thank you for choosing SCHEMA for Article! We hope that your experience with our plugin for adding SCHEMA quick and easy.', 'schema-for-article' ); ?></p>
					<p><?php printf( __( 'To support future development and help to make it even better just leaving us a <a href="%s" title="SCHEMA for Article Rating" target="_blank">%s</a> rating with a nice message to me :)', 'schema-for-article' ), 'https://wordpress.org/support/plugin/schema-for-article/reviews/?rate=5#new-post', $five_star ); ?></p>
				</div>
			</div>

			<div class="float">
        <object type="image/svg+xml" data="<?php echo $img_src;?>/schema-for-article.svg" width="128" height="128"></object>
			</div>
			...
			<div class="product">
				...
				<div class="box">
					...
					<h3><?php _e( 'JSON Structuring Markup', 'schema-for-article' ); ?></h3>
					<p><?php _e( 'Simply the easiest solution to add valid schema.org as a JSON script in the head of posts and pages. It provides you multiple <strong>SCHEMA</strong> types like Article, News Article, Organization and Website Schema.', 'schema-for-article' ); ?></p>
				...
				<div class="box">
					...
					<h3><?php _e( 'Media Post Permalink', 'schema-for-article' ); ?></h3>
					<p><?php _e( 'On uploading  any image,  let\'s say services.png, WordPress creates the <strong>attachment post</strong> with the permalink of <strong>/services/</strong> and doesn\'t allow you to use that permalink to point your page. In this case, we comes up with this great solution.', 'schema-for-article' ); ?></p>
